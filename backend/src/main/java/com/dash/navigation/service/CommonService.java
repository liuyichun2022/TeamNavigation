...
import com.dash.navigation.common.exception.ResourceReadException;
...
import com.dash.navigation.domain.entity.ISortEntity;
import com.dash.navigation.common.util.FileExtUtils;
import com.dash.navigation.common.util.QrCodeUtils;
...
public class CommonService implements ApplicationRunner {
    ...
    public void loadCardIcons() {
        CARD_ICONS.clear();
        File root = new File(TeamNavConstants.ROOT_DIR + CARD_ICON_PATH);
        if (!root.exists()) {
            return;
        }
    ...
    public String upload(MultipartFile file, String type) {
        ...
        File saveFile = new File(TeamNavConstants.ROOT_DIR + savePath);
        try {
    ...
    private String formatSavePath(String type, MultipartFile file) {
        ...
        if ("default".equals(type)) {
            ...
            Assert.isTrue(!new File(TeamNavConstants.ROOT_DIR + path).exists(), "文件名已经存在");
            return path;
        }
    ...
    public void updateIconName(String fileName, String newName) {
        String root = TeamNavConstants.ROOT_DIR + CARD_ICON_PATH;
    ...
    public void deleteDefaultIcon(String fileName) throws IOException {
        File file = new File(TeamNavConstants.ROOT_DIR + CARD_ICON_PATH + fileName);
        if (file.exists()) {
    ...
    private String getFromDocument(String domainUrl) {
        try {
            Document doc = Jsoup.connect(domainUrl)
                    .timeout(TeamNavConstants.FAVICON_TIMEOUT).get();
    ...
    private String requestFavicon(String url) {
        ...
        try {
            ...
            conn.setConnectTimeout(TeamNavConstants.FAVICON_TIMEOUT);
            conn.setReadTimeout(TeamNavConstants.FAVICON_TIMEOUT);
